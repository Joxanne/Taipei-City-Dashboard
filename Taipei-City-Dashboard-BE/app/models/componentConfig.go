...
func GetPublicComponentsForQdrant() (results []QuertChartAndConponentForQdrant, err error) {
	...
	err = DBManager.Table("query_charts as qc").
		Select("c.id, qc.index, c.name, qc.city, qc.short_desc, qc.long_desc, qc.use_case").
...
func GetComponentByQueryVector(queryString string, limit int, scoreThreshold float64) (component []CityComponentScore, err error) {
	...
	for _, item := range queryOutput {
		...
		if shortDescVal, ok := item["short_desc"]; ok && shortDescVal != nil {
			c.ShortDesc = fmt.Sprintf("%v", shortDescVal)
		}

		if longDescVal, ok := item["long_desc"]; ok && longDescVal != nil {
			c.LongDesc = fmt.Sprintf("%v", longDescVal)
		}

		if useCaseVal, ok := item["use_case"]; ok && useCaseVal != nil {
			c.UseCase = fmt.Sprintf("%v", useCaseVal)
		}

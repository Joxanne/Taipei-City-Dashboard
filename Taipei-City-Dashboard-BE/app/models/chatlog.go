...
func GetALLChatLogSession(UserID int) (chatLogList []ChatLog, err error) {
	...
	err = DBManager.Raw(`
		...
		FROM chat_logs
		WHERE user_id = ?
		ORDER BY session DESC, created_at ASC
...
func DeleteChatLogBySession(Session string, UserID int) error {
	return DBManager.
		Where("session = ? AND user_id = ?", Session, UserID).
		Delete(&ChatLog{}).Error
}

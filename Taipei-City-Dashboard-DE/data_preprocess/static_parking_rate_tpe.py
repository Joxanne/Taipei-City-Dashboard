...
try:
    load_ods = importlib.import_module("odf.opendocument").load
    _odf_table = importlib.import_module("odf.table")
    Table = _odf_table.Table
    TableRow = _odf_table.TableRow
    TableCell = _odf_table.TableCell
    P = importlib.import_module("odf.text").P
except ModuleNotFoundError:
    load_ods = None
    Table = TableRow = TableCell = P = None
...
def _expand_row(tr, max_width: int) -> list[str]:
...
def crawl_tpe() -> list[dict]:
    ...
    if load_ods is None or Table is None or TableRow is None or TableCell is None or P is None:
        log.error("[TPE] odfpy 未安裝，無法解析 ODS；請安裝 odfpy 或改用 --from-csv")
        return []
    import io
...
def _assemble_record(
    ...
    if (
        rec["weekday_rate"] is None
        and main_rate is not None
        and per_day.get("weekday", {}).get("hours_text")
    ):
...
def query_overpass(road: str, city: str, district: str | None) -> list[list[float]] | None:
    ...
    if district:
        ...
        query = OVERPASS_QUERY_CITY_ONLY.format(timeout=OVERPASS_TIMEOUT, city=city, road=road)
    try:
        res = requests.post(
            OVERPASS_URL,
            data={"data": query},
...
def _make_nominatim() -> tuple:
    ...
    geolocator = Nominatim(user_agent=USER_AGENT, timeout=NOMINATIM_TIMEOUT)  # type: ignore[arg-type]
    forward = RateLimiter(
        geolocator.geocode,
...
def geocode_all(
    ...
    if resume and csv_path.exists():
        ...
        for r in done_records:
            gkey = (
                r.get("city") or "",
                r.get("road_segment") or "",
                r.get("segment_endpoints") or "",
            )
    ...
    csv_file = open(
        csv_path, "a" if (resume and file_exists) else "w", encoding="utf-8", newline=""
    )
...
def _to_db_row(rec: dict) -> dict:
    ...
    out = dict(rec)
    out["tiered_rates_json"] = (
        json.dumps(rec.get("tiered_rates"), ensure_ascii=False)
        if rec.get("tiered_rates") is not None
        else None
    )
    out["rate_schedule_json"] = (
        json.dumps(rec.get("rate_schedule"), ensure_ascii=False)
        if rec.get("rate_schedule") is not None
        else None
    )
...
def save_to_postgres(records: list[dict]) -> None:
    ...
    valid = [
        ...
        for r in records
        if r.get("district")
        and r.get("start_lat") is not None
        and r.get("start_lon") is not None
        and r.get("geometry_path")
        and r.get("geom_source")
    ]
    if not valid:
    ...
    if len(valid) < len(records):
        log.warning(
            "[PostgreSQL] dropped %d records missing NOT NULL fields", len(records) - len(valid)
        )
...
# ── Step 5: 產生 GeoJSON（供 mapview 使用）────────────────────────────────────────
def _parking_rate_bucket(rate: int | None) -> str:
    """Convert weekday_rate into a stable legend/filter bucket."""
    if rate is None:
        return "無平日收費"
    if rate <= 30:
        return "30元以下"
...
def _rate_display(rate: int | None) -> str:
    return f"{rate} 元/時" if rate is not None else "無平日收費"


def _normalise_linestring_coords(geometry_path) -> tuple[list[list[float]], bool] | None:
    """Convert DB [[lat, lon], ...] into GeoJSON [[lon, lat], ...].

    Nominatim fallback rows may only contain one coordinate. A very short line
    segment is generated for those rows so the line layer still renders and the
    popup can show that the geometry is an approximate fallback.
    """
    if isinstance(geometry_path, str):
        try:
            geometry_path = json.loads(geometry_path)
        except ValueError:
            return None
    ...
    if len(coords) == 1:
        lon, lat = coords[0]
        # About 20-25m at Taipei latitude; used only for point fallback rows.
        delta_lon = 0.00012
        return [[lon - delta_lon, lat], [lon + delta_lon, lat]], True
    return None
...
_GEOJSON_QUERY = """
    ...
    WHERE geometry_path IS NOT NULL
      {where_extra}
    ORDER BY city, district, road_segment, ogc_fid
"""


def generate_geojson() -> None:
    """從 DB 產生停車費率道路 LineString GeoJSON，寫入 FE public/mapData/。"""
    ...
    try:
        ...
        if not cur.fetchone()[0]:
            log.warning("[GeoJSON] parking_rate_tpe 尚未建立，跳過。")
            return

        for label, where_extra, filename in [
            ("臺北市", "AND city = '臺北市'", "parking_rate_tpe.geojson"),
            ("雙北", "", "parking_rate_metrotaipei.geojson"),
        ]:
            log.info("[GeoJSON] 產生 %s 停車費率線圖層 ...", label)
            ...
            for row in rows:
                r = dict(zip(cols, row))
                normalised = _normalise_linestring_coords(r.get("geometry_path"))
                if normalised is None:
                    skipped += 1
                    continue
                coords, is_fallback_stub = normalised
                fallback_stubs += int(is_fallback_stub)
                weekday_rate = r.get("weekday_rate")
                rate_bucket = _parking_rate_bucket(weekday_rate)
                ...
                features.append(
                    {
                        ...
                        "properties": {
                            ...
                            "segment_endpoints": r.get("segment_endpoints") or "",
                            "weekday_rate": weekday_rate,
                            "saturday_rate": r.get("saturday_rate"),
                            "sunday_rate": r.get("sunday_rate"),
                            "holiday_rate": r.get("holiday_rate"),
                            "weekday_rate_text": _rate_display(weekday_rate),
                            "saturday_rate_text": _rate_display(r.get("saturday_rate")),
                            "sunday_rate_text": _rate_display(r.get("sunday_rate")),
                            "holiday_rate_text": _rate_display(r.get("holiday_rate")),
                            "weekday_hours_text": r.get("weekday_hours_text") or "",
                            "saturday_hours_text": r.get("saturday_hours_text") or "",
                            "sunday_hours_text": r.get("sunday_hours_text") or "",
                            "holiday_hours_text": r.get("holiday_hours_text") or "",
                            "rate_text": r.get("rate_text") or "",
                            "rate_bucket": rate_bucket,
                            "geom_source": r.get("geom_source") or "",
                            "geometry_note": "定位點近似線段" if is_fallback_stub else "道路線段",
                        },
                    }
                )
            ...
            log.info(
                "[GeoJSON] %s：%d 條線（%d 筆定位點近似、略過 %d 筆）→ %s",
                filename,
                len(features),
                fallback_stubs,
                skipped,
...
def _smoke_test_helpers(with_network: bool = False) -> None:
    ...
    if with_network:
        ...
        assert {"county", "area", "road_name", "rates", "_source"}.issubset(
            rows[0].keys()
        ...
        assert (
            coords is not None and len(coords) >= 2
        ), f"Overpass should return polyline for 信義路四段 in 大安區, got {coords}"
        assert all(
            120 < c[1] < 122 and 24 < c[0] < 26 for c in coords
        ...
        log.info(
            "[OK] geocode_one NTPC-style passed (source=%s, points=%d).",
            ntpc_sample["geom_source"],
            len(ntpc_sample["geometry_path"]),
        )
        time.sleep(OVERPASS_DELAY_SEC)
        ...
        assert tpe_sample["district"] and tpe_sample["district"].endswith(
            "區"
        ), f"reverse geocode should fill district, got {tpe_sample['district']!r}"
        log.info(
            "[OK] geocode_one TPE-style passed (district=%s, source=%s).",
            tpe_sample["district"],
            tpe_sample["geom_source"],
        )
        time.sleep(OVERPASS_DELAY_SEC)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="只跑 crawl + preprocess + geocode，不寫入 DB"
    )
    parser.add_argument("--limit", type=int, default=None, help="處理前 N 筆（debug 用）")
    parser.add_argument(
        "--csv-path",
        type=str,
    ...
    parser.add_argument(
        "--no-resume", action="store_true", help="不從 CSV 接續，重新跑全部（會覆蓋 CSV）"
    )
    parser.add_argument(
        "--from-csv",
        type=str,
    ...
    parser.add_argument(
        "--generate-geojson",
        action="store_true",
        help="只從 parking_rate_tpe 產生 FE mapData GeoJSON 後結束",
    )
    parser.add_argument(
        "--skip-geojson",
        action="store_true",
        help="寫入 DB 後不要產生 FE mapData GeoJSON",
    )
    # Debug / smoke flags
    parser.add_argument(
        "--smoke-test", action="store_true", help="Run inline helper smoke tests and exit."
    )
    parser.add_argument(
        "--with-network", action="store_true", help="Also exercise live HTTP smoke checks."
    )
    parser.add_argument(
        "--with-db",
        action="store_true",
        ...
        fake = [
            {
                ...
                "city": "新北市",
                "district": "板橋區",
                "road_segment": "__SMOKE_TEST__",
                "segment_endpoints": None,
                "weekday_rate": 30,
                "saturday_rate": None,
                "sunday_rate": None,
                "holiday_rate": None,
                "weekday_start_time": dtime(7, 0),
                "weekday_end_time": dtime(20, 0),
                ...
                "weekday_hours_text": "07:00~20:00",
                "saturday_hours_text": None,
                "sunday_hours_text": None,
                "holiday_hours_text": None,
                "tiered_rates": None,
                "rate_schedule": {
                    "weekday": {
                        "charging": True,

...
bus_stop_by_district	{#0EA5E9,#0284C7,#0369A1,#075985,#0C4A6E,#BAE6FD,#7DD3FC,#38BDF8}	{DistrictChart,ColumnChart}	站
bus_avg_estimate_by_district	{#ef4444,#f97316,#eab308,#22c55e,#3b82f6,#8b5cf6,#ec4899,#06b6d4}	{DistrictChart,ColumnChart}	分
\.
...
110	bus_stop_estimate_tpe	公車站點	circle	geojson	\N	\N	{"circle-radius":4,"circle-opacity":0.8,"circle-color":["case",["<",["get","nearest_estimate_min"],0],"#94a3b8",["<=",["get","nearest_estimate_min"],3],"#ef4444",["<=",["get","nearest_estimate_min"],6],"#f97316",["<=",["get","nearest_estimate_min"],10],"#eab308","#22c55e"]}	[{"key":"stop_name","name":"站牌名稱"},{"key":"district","name":"行政區"},{"key":"geo_city","name":"城市"},{"key":"nearest_estimate_min","name":"最近到站(分)"}]
111	bus_stop_estimate_metrotaipei	公車站點	circle	geojson	\N	\N	{"circle-radius":4,"circle-opacity":0.8,"circle-color":["case",["<",["get","nearest_estimate_min"],0],"#94a3b8",["<=",["get","nearest_estimate_min"],3],"#ef4444",["<=",["get","nearest_estimate_min"],6],"#f97316",["<=",["get","nearest_estimate_min"],10],"#eab308","#22c55e"]}	[{"key":"stop_name","name":"站牌名稱"},{"key":"district","name":"行政區"},{"key":"geo_city","name":"城市"},{"key":"nearest_estimate_min","name":"最近到站(分)"}]
\.
...
301	bus_stop_by_district	行政區公車站牌數量
302	bus_avg_estimate_by_district	行政區公車平均候車時間
\.
...
370	bus_stop_tpe	公車站牌分布	{301}	directions_bus	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00
371	bus_stop_newtpe	公車站牌分布	{301}	directions_bus	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00
372	bus_estimate_tpe	公車候車時間	{302}	directions_bus	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00
373	bus_estimate_newtpe	公車候車時間	{302}	directions_bus	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00
\.
...
bus_avg_estimate_by_district	\N	{110}	\N	current	\N	5	minute	tcgbusfs	各行政區公車平均候車時間（臺北市）	依行政區統計臺北市公車站牌的平均預估到站時間（僅計算有發車的班次）。	可用於分析各行政區公車服務頻率與候車便利性差異。	{}	{}	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00	two_d	SELECT bst.district AS x_axis,\r\n       ROUND(AVG(bet.estimate_time) / 60.0, 1) AS data\r\nFROM public.bus_estimate_time_tpe bet\r\nJOIN public.bus_route_stops_tpe brs\r\n     ON bet.city = brs.city AND bet.stop_id = brs.stop_id\r\nJOIN public.bus_stop_tpe bst\r\n     ON brs.city = bst.city AND brs.stop_location_id = bst.stop_location_id\r\nWHERE bet.estimate_time > 0\r\n  AND bst.geo_city = '臺北市'\r\n  AND bst.district IS NOT NULL\r\nGROUP BY bst.district\r\nORDER BY data	\N	taipei
bus_avg_estimate_by_district	\N	{110,111}	\N	current	\N	5	minute	tcgbusfs/ntpcbus	各行政區公車平均候車時間（雙北）	依行政區統計雙北公車站牌的平均預估到站時間（僅計算有發車的班次）。	可用於比較雙北各行政區公車服務頻率與候車便利性差異。	{}	{}	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00	two_d	SELECT bst.district AS x_axis,\r\n       ROUND(AVG(bet.estimate_time) / 60.0, 1) AS data\r\nFROM public.bus_estimate_time_tpe bet\r\nJOIN public.bus_route_stops_tpe brs\r\n     ON bet.city = brs.city AND bet.stop_id = brs.stop_id\r\nJOIN public.bus_stop_tpe bst\r\n     ON brs.city = bst.city AND brs.stop_location_id = bst.stop_location_id\r\nWHERE bet.estimate_time > 0\r\n  AND bst.district IS NOT NULL\r\nGROUP BY bst.district\r\nORDER BY data	\N	metrotaipei
bus_stop_by_district	\N	\N	\N	static	\N	\N	\N	tcgbusfs	各行政區公車站牌數量（臺北市）	依行政區統計臺北市公車物理站牌數量，反映各區公車服務覆蓋密度。	可用於分析各行政區的公車服務分布情形，輔助交通資源規劃。	{}	{}	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00	two_d	SELECT district AS x_axis, COUNT(*) AS data\r\nFROM public.bus_stop_tpe\r\nWHERE geo_city = '臺北市' AND district IS NOT NULL\r\nGROUP BY district\r\nORDER BY data DESC	\N	taipei
bus_stop_by_district	\N	\N	\N	static	\N	\N	\N	tcgbusfs/ntpcbus	各行政區公車站牌數量（雙北）	依行政區統計雙北公車物理站牌數量，反映各區公車服務覆蓋密度。	可用於分析各行政區的公車服務分布情形，輔助交通資源規劃。	{}	{}	2025-01-01 00:00:00+00	2025-01-01 00:00:00+00	two_d	SELECT district AS x_axis, COUNT(*) AS data\r\nFROM public.bus_stop_tpe\r\nWHERE district IS NOT NULL\r\nGROUP BY district\r\nORDER BY data DESC	\N	metrotaipei
\.

...
import BusRouteChart from "./components/BusRouteChart.vue";
import BusTransferChart from "./components/BusTransferChart.vue";
...
function returnChartComponent(name, svg) {
	switch (name) {
		...
		case "IsochroneTool":
			return svg ? MapLegendSvg : IsochroneTool;
		case "BusRouteChart":
			return svg ? MetroChartSvg : BusRouteChart;
		case "BusTransferChart":
			return svg ? MetroChartSvg : BusTransferChart;
		default:
			return svg ? MapLegendSvg : MapLegend;
	}
}
...
<template>
	...
	>
		...
		<div
			v-else-if="
				(config.chart_data || hasCustomChart) &&
			...
			:class="{
				...
				'dashboardcomponent-chart-free':
					hasCustomChart && activeChart === 'BusTransferChart',
			}"
		>
			<component
